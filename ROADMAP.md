...
| 0.2 | Prototype anchors (tree-sitter symbols + content fingerprints); replay real historical refactor commits against them | ≥80% anchor survival across rename + file-split refactors | ✅ **PASSED 2026-06-09** (prototype), **re-validated 2026-07-01 on the shipped tree-sitter mechanism** — 94.6% single-commit, 92.5% at 200-commit zero-maintenance horizon; naive path+line baseline 0.0% ([results](experiments/anchor-survival/RESULTS.md)). Numbers measure **symbol-anchor** survival, which requires the `scar-cli[symbols]` extra |
...
| 0.4 | Auto-authorship trial: 2 weeks of normal agent-assisted work with the stop-hook drafting `deadend` candidates | ≥5 human-kept scars; false-positive rate <15% (skeptic's bar, adopted) | ✅ **Closed early 2026-06-11** (day 3 of 14). Per [FINDINGS](experiments/auto-authorship/FINDINGS.md): 3 drafter candidates kept after review against the ≥5 bar, plus 2 organic scars authored outside the drafter flow; candidates that reached review were kept at a high rate. One agent-authored scar caught a real parser bug and fired on the exact edit that fixed it. Drafter *trigger* precision failed its own bar (2 of 4 firings false) and was retuned to revert-language-only as a result. |
...
- ✅ CLI: `init, lint, status, promote, check, why, challenge, archive, orphan, harvest, hook, skill, mcp, agent, inject` — lifecycle commands (`challenge`/`archive`, expiry review surfacing) shipped beyond the original plan
- ✅ Claude Code plugin: PreToolUse injection + stop-hook candidate drafting, both field-validated (gates 0.3, 0.4)
- ⚠️ **Python, not Go/Rust** — a stdlib-only parser/hook hot-path hits the goal the compiled binary was chasing (~20ms hook startup, trivial install via `uv tool install scar-cli`); the human-facing CLI adds `rich`/`rich-argparse` for output only. A rewrite is not planned unless profiling says otherwise
